...
class RefactorAgent:
    ...
    def refactor_task_package(self, designer_package_path: Path, refactored_comp_path: Path, raw_dataset_path: Path):
        ...
        # 2. 将原始数据集复制到 raw/ 目录
        destination_path = raw_dir / "train.csv"
        shutil.copy(raw_dataset_path, destination_path)
        print(f">>> Refactor: 已将原始数据 {raw_dataset_path.name} 复制并重命名为 train.csv")
...
class MLESmithPipeline:
    def __init__(self, workspace_dir: str = "./mle_smith_workspace"):
        ...
        self.harvester_output_path = self.workspace_path / "harvester_output"
...
""", encoding='utf-8')
    ...
    def run(self, source: str):
        ...
        harvested_assets = self.harvester.harvest_from_source(source)

        if not harvested_assets or not any(harvested_assets.values()):
            print("Harvester 未能从源中采集到任何有效资产，流水线终止。")
            return

        # --- 第 2 步: 遍历所有有价值的表格，生成ML任务 ---
        if not harvested_assets["tables"]:
            print("\n未发现适合生成机器学习任务的表格。")
        else:
            print(f"\n>>> Pipeline: 发现 {len(harvested_assets['tables'])} 个有价值的表格。开始逐一生成任务...")
            for i, table_info in enumerate(harvested_assets["tables"]):
                print(
                    f"\n--- 处理表格 {i + 1}/{len(harvested_assets['tables'])}: {table_info['suggested_filename']} ---")
                print(f"    理由: {table_info['justification']}")

                table_path = table_info['path']

                # --- 2a. Brainstormer ---
                proposals = self.brainstormer.brainstorm_tasks(table_path)
                if not proposals:
                    print(f"Brainstormer 未能为表格 {table_info['suggested_filename']} 生成构思，跳过。")
                    continue
        ...
        for i, table_info in enumerate(harvested_assets["tables"]):
            print(
                f"\n{'=' * 20} 处理表格 {i + 1}/{len(harvested_assets['tables'])}: {table_info['suggested_filename']} {'=' * 20}")
            print(f"    理由: {table_info['justification']}")
            ...
            # --- 内部循环: 处理为单个表格生成的所有任务构思 ---
            for j, proposal in enumerate(proposals):
                print(f"\n--- 处理构思 {j + 1}/{len(proposals)} for table {i + 1} ---")
                proposal.pretty_print()

                # --- 步骤 2b: Designer ---
                designer_package_name = f"table_{i + 1}_proposal_{j + 1}_designer"
                ...
                if not success:
                    print(f"Designer 未能创建任务包，跳过此构思。")
                    continue

                # --- 步骤 2c: Refactor ---
                refactored_comp_name = f"comp_t{i + 1}_p{j + 1}_{proposal.prediction_target.lower()}"
                refactored_comp_path = self.refactor_output_path / refactored_comp_name
                refactor_success = self.refactor.refactor_task_package(designer_package_path, refactored_comp_path,
                                                                       table_path)
                if not refactor_success:
                    print(f"Refactor 未能重构任务包，跳过此构思。")
                    continue

                # --- 步骤 2d: Verifier ---
                print(f"\n--- 开始验证任务: {refactored_comp_name} ---")
        ...
        print("\n\n--- 其他资产采集摘要 ---")
        if harvested_assets["charts"]:
            print(f"采集到 {len(harvested_assets['charts'])} 个图表的分析报告。详情请见 harvester_output 目录。")
        if harvested_assets["code_snippets"]:
            print(f"采集到 {len(harvested_assets['code_snippets'])} 个代码片段。详情请见 harvester_output 目录。")
...
if __name__ == "__main__":
    ...
    if "YOUR_API_KEY_HERE" in os.environ.get("DEEPSEEK_API_KEY", ""):
        ...
        try:
            ...
            source_url = "https://doi.org/10.5281/zenodo.14049120"
